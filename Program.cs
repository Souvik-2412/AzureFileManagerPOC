...
// Read config from environment variables (set in App Service Configuration)
...
if (string.IsNullOrEmpty(blobConnectionString) || string.IsNullOrEmpty(tableConnectionString))
{
    throw new Exception("Required environment variables are missing: BLOB_CONN_STRING or TABLE_CONN_STRING.");
}
